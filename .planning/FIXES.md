...
| FIX-15 | SSH credential handed to the model and written to `chats.db` in the clear | `server.js:2175` (prompt block), `serializeMessageAttachments`, `saveInterruptAttachments` | Found while reviewing #23, **not introduced by it** — `git log -S` puts it in `215848e` (v5.25.4). Three surfaces, each outliving the chat: the prompt text block (passed as `-p` and written to the CLI transcript jsonl), `messages.attachments` (plaintext column), and the MCP interrupt payload (a tool result the model reads). Bought nothing — there is no `sshpass` in this project, so a password in the prompt is unusable; real SSH runs server-side in `claude-ssh.js` from the host entry. |
...
| FIX-05 | **FIXED** | `1588b4b` + `5962d87` closed `terminal-session.test.js`; `497cfc8` (PR #41) closed `terminal-bridge.integration.test.js` by replacing all 14 fixed sleeps with `waitFor(pred, {timeoutMs, intervalMs})` polls. Proven under load, not on an idle machine: 5/5 runs `40 passed, 0 failed` with 10 cores of `yes` running (loadavg 86→122); runtime also dropped 25.7s → 6.3s. Assertion count unchanged at 36. GitHub **#39** closed. |
...
| FIX-08 | **FIXED** | `741ed67`. **Correction to this document:** the heading says 16 files; the count has since moved twice as new regression suites landed. As of `cd62225` it is **30** (10 `test/render/*.test.mjs` via `node --test` + 20 `test/*.test.js`), which is what `CLAUDE.md` states. |
...
| FIX-10 | **FIXED** | `test/setup-gate.test.js` (28 assertions) covers the bind, the setup gate and the trust-proxy default; `test/path-guard.test.js` (24 assertions) closes the two gaps this row named. It drives a real server on a throwaway data dir and asserts `isPathAllowed` through the endpoints that call it — including a symlink inside WORKDIR pointing at `/etc`, which lexical containment alone lets through — plus a positive control so a guard that refused everything could not pass. The tunnel rule is proven with a fake `cloudflared` on `PATH`: with the tunnel up, `/ws/terminal` refuses and `/api/terminal/capability` reports `term.off.tunnel`; both lift when it stops. Four mutations, each caught by a named assertion: `isPathAllowed` → `true` (8 FAILs, one of them a write that reached `/etc` and returned 500), dropping `_realish` (exactly the 2 symlink assertions), removing the socket guard, removing the capability branch (`available` flipped to `true` while a tunnel was live). |
...
| FIX-14 | **FIXED** | All of I-3…I-9 (GitHub #30–#36) closed. Two findings were split out rather than folded in: **#38** (`noCatchUp` never cancels `TASK_DISCONNECT_TIMEOUT_MS`) and **#39** (above). Both are now closed too — #38 by `d6299ef` (PR #42), which hoists the recovery block and only re-attaches the stream when no live socket holds it, guarded by `test/subscribe-nocatchup.test.js`; #39 by `497cfc8` (PR #41). |
| FIX-15 | **FIXED** | `test/ssh-secret.test.js` (26 assertions). The credential is no longer interpolated into the prompt (the block now says `Auth: password (held server-side, not exposed)`), is stripped on the SQLite write path, and is replaced by an `authMethod` label in the MCP interrupt payload. Proven against a real server with a fake `claude` that records its own argv and polls the internal interrupt endpoint exactly as `mcp-user-interrupt.js` does. Every leak assertion is paired with a positive control, because "the password is absent" also holds when the SSH path never ran. Three mutations, each caught by a named assertion: restoring the prompt interpolation (3 FAILs), dropping the SQLite strip (2 FAILs — one of them `data/chats.db-wal` caught by the whole-data-directory sweep), handing the field back to the MCP subprocess (2 FAILs). |
