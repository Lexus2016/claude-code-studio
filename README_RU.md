...
> **v5.65.0** — Устойчивость и навигация: ответы больше не падают, когда сервер временно перегружен (HTTP 429/529) — они делают паузу и автоматически повторяются; а таймаут подпроцесса теперь по бездействию, поэтому долгий, но активный запуск никогда не обрывается посреди потока (`CLAUDE_IDLE_TIMEOUT_MS` / `CLAUDE_HARD_CAP_MS`). Плюс мини-карта сообщений, живая панель активности и объединённое меню экспорта сессии.
