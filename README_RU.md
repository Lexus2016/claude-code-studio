...
> **v5.71.0** — **Усиление безопасности и надёжности.** Восстановлен SSH-движок чата (регрессия ломала сообщения в SSH-проектах), desktop-сборка больше не упаковывает локальные секреты, добавлены защита от падения WebSocket и JSON-обработчик ошибок API, права auth-файлов сужены до `0600`, подключены автоматические тесты (`npm test`).
