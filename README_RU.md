...
> **v7.9.0** — **Зависший удалённый чат больше не зависает навсегда.** Удалённый SSH-чат мог заблокироваться насовсем при обрыве соединения посреди работы — новые сообщения отклонялись, а даже **«Restart Session»** отвечал «Task is still running». Исправление меняет то, как завершается удалённый запуск, чтобы ничего не оставалось недоделанным, и теперь Restart Session восстанавливает зависший чат, а не отказывает. Также в этом релизе: сломанная или отсутствующая локальная установка Claude CLI теперь обнаруживается и показывается в интерфейсе **до** того, как из-за неё оборвётся отправка сообщения, и закрыты два небольших пробела в Telegram-боте — заработала команда `/cancel`, а проект, добавленный после начального подключения, сразу получает свою тему в форуме.
>
