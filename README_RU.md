...
> **v5.68.0** — Живой индикатор активности в веб-чате: в потоковом ответе теперь видно, что делает Claude — «Думаю…», «Выполняю: <инструмент>…», «Генерирую…» — с живым счётчиком времени в подвале сообщения, так что всегда понятно, что работа идёт и сколько уже длится. Включает исправление: таймер больше не «утекает» при закрытии вкладки или смене сессии во время ответа.
