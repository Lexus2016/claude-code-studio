...
**Топики проектов** — каждая директория проекта получает свой тред. Пишите сообщение в топик проекта — Claude работает в его контексте. `/new` — новая сессия, `/history` — прошлые сессии с кнопками переключения, `/session N` — переключиться, `/files` — файлы, `/cat` — содержимое файла, `/diff` — изменения, `/log` — git-история, `/last` и `/full` — последние сообщения, `/stop` — остановить. Каждое done/error сообщение имеет контекстные кнопки — [📜 History] [🆕 New] — навигация без команд. `/help` в любом топике — полная справка.

**Топик Tasks** — ваш Kanban прямо в Telegram. Напишите название задачи — она создана мгновенно. Полное управление жизненным циклом: `/list` — все задачи, `/start ID` — начать, `/done ID` — завершить, `/todo ID`, `/block ID`, `/backlog ID` — изменить статус, `/delete ID` — удалить. Управляйте всем бэклогом без открытия браузера.

**Топик Activity** — каждое уведомление (задача выполнена, упала, сессионные события) постится с интерактивными кнопками: [💬 Open chat] — переход к сессии в топике проекта, [📁 Project] — навигация к треду проекта. Не просто лог — центр навигации.
...
### 📊 Dashboard — Ваша продуктивность одним взглядом

Насколько эффективен ваш AI-рабочий процесс? Страница Dashboard даёт визуальный ответ — без догадок, только данные.

![Dashboard](public/screenshots/dashboard.jpg)

**Что вы видите:**
- **Hero-карточки** — общее количество сессий, вызовов инструментов, сообщений и примерное сэкономленное время (на основе среднего времени ручного выполнения каждой операции)
- **Heatmap активности** — сетка в стиле GitHub за последние 90 дней
- **Использование инструментов** — какие инструменты Claude делают основную работу (Read, Bash, Edit, Grep...) — айсберг скрытой автоматизации
- **Automation Index** — оценка 0–100, сочетающая долю инструментов в сообщениях, использование мульти-агентного режима и стабильность активности
- **Распределение моделей** — круговая диаграмма Haiku / Sonnet / Opus
- **Пиковые часы** — когда вы наиболее активны, по часам
- **Топ сессий** — самые интенсивные разговоры, ранжированные по количеству сообщений, с бейджами проектов и навигацией в один клик обратно к чату

Каждое число ссылается на реальные данные. Нажмите на топ-сессию — и вы попадёте прямо в этот разговор, в правильном контексте проекта.

Dashboard — это маркетинговый инструмент для вашей собственной продуктивности. Тот экран, который хочется заскриншотить и поделиться.
...
| 📊 Dashboard | Визуальная аналитика — heatmap активности, использование инструментов, распределение моделей, индекс автоматизации, топ сессий с навигацией в один клик |
