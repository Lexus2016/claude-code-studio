...
> **v7.2.3** — **Обновление из приложения на macOS больше не зацикливается.** Приложение сверялось с релизом на GitHub, который публикуется сразу при создании тега, тогда как cask в Homebrew — откуда оно на самом деле ставится — обновляется примерно на 8 минут позже. В этом промежутке оно предлагало версию, которую brew ещё не мог установить, закрывалось и возвращалось на старой. Теперь проверяется cask, а обновление, ничего не изменившее, считается неудачей и показывает уведомление вместо молчаливого перезапуска.
