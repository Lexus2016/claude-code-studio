...
**Быстрое прикрепление через `#`** — введите `#` в поле чата, появится попап с вашими настроенными SSH-серверами. Выберите нужный — он прикрепится как чип, и Claude будет знать, что команды нужно выполнять на той машине. Без переключения контекста, без кликов по сайдбару — просто `#` и вперёд.
