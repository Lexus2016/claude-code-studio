...
> **v7.3.0** — **Настройки, которые наконец видно.** Форма настроек показывает каждый параметр рядом с тем местом, откуда на самом деле берётся его значение — переменная оболочки, `.env`, локальный `config.json`, глобальный, переопределение для проекта или встроенное значение по умолчанию, — отмечает те, что перекрыты или молча игнорируются, маскирует каждый секрет ещё на сервере и возвращает любой параметр к значению по умолчанию одним кликом. Также в этом релизе: импорт сессий Claude CLI с **удалённого SSH-хоста**, ответ на **заблокированный запрос разрешения или плана** прямо из браузера вместо перехода в терминал, **глобальное рабочее пространство** с поиском задач и файлов сразу по всем проектам, и перевод последних непереведённых участков интерфейса на все пять языков. Безопасность: SSH-пароль или ключ никогда не попадает ни в модель, ни в транскрипт, ни в базу.
>
...
Передавайте задачи внешним AI CLI — OpenAI Codex, Grok CLI, Antigravity CLI, opencode, Hermes, Kimi — прямо из интерфейса чата. Два режима:
...
| **Удалённый доступ** | SSH-серверы, SFTP-загрузка, быстрое подключение `#`, туннели cloudflared/ngrok, импорт удалённых сессий CLI |
