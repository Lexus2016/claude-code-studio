...
> **v5.71.0** — **Зміцнення безпеки та надійності.** Відновлено SSH-рушій чату (регресія ламала повідомлення в SSH-проєктах), desktop-білд більше не запаковує локальні секрети, додано захист від падіння WebSocket і JSON-обробник помилок API, права auth-файлів звужено до `0600`, підключено автоматичні тести (`npm test`).
