...
**Топіки проєктів** — кожна директорія проєкту отримує свій тред. Пишіть повідомлення у топік проєкту — Claude працює в його контексті. `/new` — нова сесія, `/history` — минулі сесії з кнопками перемикання, `/session N` — перемкнутись, `/files` — файли, `/cat` — вміст файлу, `/diff` — зміни, `/log` — git-історія, `/last` і `/full` — останні повідомлення, `/stop` — зупинити. Кожне done/error повідомлення має контекстні кнопки — [📜 History] [🆕 New] — навігація без команд. `/help` в будь-якому топіку — повна довідка.

**Топік Tasks** — ваш Kanban прямо в Telegram. Напишіть назву задачі — вона створена миттєво. Повне управління життєвим циклом: `/list` — всі задачі, `/start ID` — розпочати, `/done ID` — завершити, `/todo ID`, `/block ID`, `/backlog ID` — змінити статус, `/delete ID` — видалити. Керуйте всім беклогом без відкриття браузера.

**Топік Activity** — кожне сповіщення (задача виконана, впала, сесійні події) поститься з інтерактивними кнопками: [💬 Open chat] — перехід до сесії в топіку проєкту, [📁 Project] — навігація до треду проєкту. Не просто лог — центр навігації.
...
### 📊 Dashboard — Ваша продуктивність одним поглядом

Наскільки ефективний ваш AI-робочий процес? Сторінка Dashboard дає візуальну відповідь — без здогадок, тільки дані.

![Dashboard](public/screenshots/dashboard.jpg)

**Що ви бачите:**
- **Hero-картки** — загальна кількість сесій, викликів інструментів, повідомлень та орієнтовний заощаджений час (на основі середнього часу ручного виконання кожної операції)
- **Heatmap активності** — GitHub-стиль сітка активності за останні 90 днів
- **Використання інструментів** — які інструменти Claude роблять основну роботу (Read, Bash, Edit, Grep...) — айсберг прихованої автоматизації
- **Automation Index** — оцінка 0–100, що поєднує відношення інструментів до повідомлень, використання мульти-агентного режиму та стабільність активності
- **Розподіл моделей** — кругова діаграма Haiku / Sonnet / Opus
- **Пікові години** — коли ви найактивніші, по годинах
- **Топ сесій** — ваші найінтенсивніші розмови, ранжовані за кількістю повідомлень, з бейджами проєктів та навігацією в один клік назад до чату

Кожне число посилається на реальні дані. Натисніть на топ-сесію і ви потрапите прямо в цю розмову, в правильному контексті проєкту.

Dashboard — це маркетинговий інструмент для вашої власної продуктивності. Той екран, який хочеться заскріншотити і поділитися.
...
| 📊 Dashboard | Візуальна аналітика — heatmap активності, використання інструментів, розподіл моделей, індекс автоматизації, топ сесій з навігацією в один клік |
