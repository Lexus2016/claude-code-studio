...
> **v5.65.0** — Стійкість і навігація: відповіді більше не падають, коли сервер тимчасово перевантажений (HTTP 429/529) — вони роблять паузу й автоматично повторюються; а таймаут підпроцесу тепер за бездіяльністю, тож довгий, але активний запуск ніколи не обривається посеред потоку (`CLAUDE_IDLE_TIMEOUT_MS` / `CLAUDE_HARD_CAP_MS`). Плюс мінімапа повідомлень, жива панель активності та об'єднане меню експорту сесії.
