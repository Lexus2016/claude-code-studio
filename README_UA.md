...
> **v5.58.0** — Розмір шрифту повідомлень на льоту (`=` більше, `-` менше, `0` скидання, зберігається між сесіями); недовідправлені чернетки тепер виживають перезавантаження сторінки — ніколи не втрачайте наполовину написаний запит.
...
**Переходите з консольної/веб-версії?** У застосунку: **File → Import data from CLI / web version…** — вкажіть теку старої студії, і вона перенесе історію чатів, налаштування (`config.json`) та навички (спершу робиться бекап, потім застосунок перезапускається). Без ручного копіювання файлів.
