...
`npm test` runs 57 test files under `test/` (18 `test/render/*.test.mjs` + 39 `test/*.test.js`). There is no CI yet, and nothing covers the live browser/WebSocket path, so also verify that manually:
